...
A External Module for the deprecating a class's subclassing features
without needing to place wrapper around a `__init_subclass__` or
needing to define it. It also has backwards compatability for `3.10`
and onwards.
::
...
def join_version_if_sequence(
    ver: str | Sequence[int] | Iterable[int] | tuple[int, ...],
) -> str:
...
class deprecated_subclass:
    ...
    def __init__(
        ...
        *,
        ...
        stacklevel: int = 1,
        removed_in: str | Sequence[int] | tuple[int, ...] | None = None,
    ) -> None:
        """
